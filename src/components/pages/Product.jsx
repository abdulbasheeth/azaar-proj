...
const Product = () => {
    ...
    // Scroll to product section if navigated from home
    useEffect(() => {
                    ...
                    <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide mt-10">
                        ...
                        {categories.map(category => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap border-b-2 ${activeCategory === category
                                    ? "border-blue-300 bg-[#004D99] text-white"
                                    : "border-transparent text-muted-foreground hover:border-blue-200"
                                    }`}
                            >
                                {category}{category !== "All" && <span className="ml-1.5 opacity-70">({products.filter(p => p.category === category).length})</span>}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <AnimatePresence>
                {selectedProduct && (
                    ...
                    >
                        ...
                        >
                            <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white hover:bg-black/70 transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                            ...
                            <div className="grid md:grid-cols-2 border-none">
                                ...
                                <div className="p-8 flex flex-col justify-center">
                                    <span className={`inline-block w-fit text-xs px-3 py-1 rounded-full font-medium border mb-4 ${categoryColors[selectedProduct.category]}`}>{selectedProduct.category}</span>
